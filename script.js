...
const chapterBlueprints = [
  ...
  },
  {
    ...
    activities: [
      ...
      {
        title: 'Code Studio',
        mode: 'pythonCodeStudio',
        status: 'Available now',
        goal: 'Write and run complete Python using fixed test data, then inspect output and repair actual errors.',
        misconception: 'A program that runs is not automatically correct. Use a boundary or contrasting test value as evidence.',
        challenge: 'Complete a fresh DSE-style mini task, check the deterministic sample output, then export a PNG evidence card.',
        transfer: 'DSE transfer: translate a brief into variables, selection or iteration, then test the required output.'
      }
    ],
    practice: [
      {
  ...
  },
  {
    ...
    activities: [
      ...
      },
      {
        ...
        mode: 'sqlCodeStudio',
        status: 'Available now',
        goal: 'Write and execute complete SQL against a temporary Student table, including SELECT, UPDATE and INSERT.',
        misconception: 'Valid SQL is not enough: the result table or final database state must answer the exact task safely.',
        challenge: 'Complete a DSE-style task, inspect the result set, then export a PNG evidence card for submission.',
        transfer: 'DSE transfer: use fields, conditions, values and a follow-up SELECT as concrete evidence for SQL correctness.'
      }
    ],
    practice: [
      {
